...
const reviewModel = {
  /**
   * Find reviews by destination ID, package ID, or user ID
   */
  async findAll({ destinationId, packageId, userId, limit = 50, offset = 0 } = {}) {
    try {
      let sql = `
        SELECT 
          r.*,
   ...
   */
  async checkUserBookingEligibility(userId, { destinationId, packageId } = {}) {
    const uid = parseInt(userId, 10);
    try {
      let sql = `
        SELECT id, booking_reference, status, travel_date
        FROM bookings
      ...
      // In fallback mode, grant eligibility if user has a booking or for demonstration
      return {
        isEligible: true,
        bookingId: 1,
